...
public class ParticleAdditiveCompositor {
	...
	public void compositeOnCanvas(Particles particles) {
		...
		for (int i = 0; i < particles.size(); i++) {
			...
			int topLeftX = (int) Math.floor(pos.getX()) - sw / 2, topLeftY = (int) Math.floor(pos.getY()) - sh / 2,
		...
		for (int i = 0; i < particles.size(); i++) {
			...
			for (int y = 0; y < sh; y++, dy++) {
				...
				for (int x = 0; x < sw; x++, dx++) {
					 ...
					 * Hand-unrolled:
					 * 
					 * for (int c = 0; c < 3; c++) { int value = ((dst >> (8 * c)) & 0xff) + (alpha
					 * * ((src >> (8 * c)) & 0xff) / 255); if (value > 255) { value = 255; } res |=
					 * value << (8 * c); }
					 * 
					 * int intensity = (dst & 0xFF) / 5 + ((dst >> 8) & 0xFF) * 7 / 10 + ((dst >>
					 * 16) & 0xFF) * 7 / 10;
					 * 
		...
		SampleModel sm = new SinglePixelPackedSampleModel(DataBuffer.TYPE_INT, bw, bh,
	...
	public void compose(Graphics g, Particles particles) {
		...
		/*
		 * g.setColor(Color.RED); g.drawRect(minX, minY, maxX-minX, maxY-minY);
		 */
	}
